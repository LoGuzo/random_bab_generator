...
int parse_members_from_text(const char*, LPARRAY); // 이름 추출 함수
char* merge_members_text(LPARRAY, int); // text합병 함수

// api 처리함수
void request_API(SlackChannel*, LPARRAY, LPARRAY); // api 요청 함수
char* get_token_from_file(const char*); // bot_token 확인 함수
int slack_fetch_channels(const char*, SlackChannel*, int*); // 채널 가져오기 함수
void slack_user_name_by_id(const char*, const char*, LPARRAY); // 이름 추출 함수

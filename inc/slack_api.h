...
size_t writefunc(void *, size_t, size_t,
                 struct string *);                  // writefunc 제작 함수
...
void slack_user_name_by_id(const char *, const char *,
                           LPARRAY); // id에서 이름 추출 함수
void slack_conversation_members(const char *, const char *,
                                LPARRAY); // 인원 추출 함수
void slack_send_message(char *, const char *,
                        const char *); // 메세지 보내기 함수
void slack_recent_message(char *, const char *,
                          LPARRAY); // 최근 메세지 가져오기 함수
